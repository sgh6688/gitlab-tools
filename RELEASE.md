...
gitlab_tools/__init__.py: 0.3.3
```
...
```text
dist/gitlab_tools-0.3.3-py3-none-any.whl
```
...
/tmp/gitlab-tools-release-check/bin/python -m pip install --no-index dist/gitlab_tools-0.3.3-py3-none-any.whl
...
release-check\Scripts\python -m pip install --no-index dist\gitlab_tools-0.3.3-py3-none-any.whl
...
```bash
(cd dist && shasum -a 256 gitlab_tools-0.3.3-py3-none-any.whl > SHA256SUMS.txt)
```

Linux：

```bash
(cd dist && sha256sum gitlab_tools-0.3.3-py3-none-any.whl > SHA256SUMS.txt)
```

Windows PowerShell：

```powershell
$wheel = "dist\gitlab_tools-0.3.3-py3-none-any.whl"
...
```bash
git tag -a v0.3.3 -m "gitlab-tools v0.3.3"
