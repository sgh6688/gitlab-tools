...
gitlab_tools/__init__.py: 0.3.1
```
...
```text
dist/gitlab_tools-0.3.1-py3-none-any.whl
```
...
/tmp/gitlab-tools-release-check/bin/python -m pip install --no-index dist/gitlab_tools-0.3.1-py3-none-any.whl
...
release-check\Scripts\python -m pip install --no-index dist\gitlab_tools-0.3.1-py3-none-any.whl
...
```bash
shasum -a 256 dist/gitlab_tools-0.3.1-py3-none-any.whl > dist/SHA256SUMS.txt
```

Linux：

```bash
sha256sum dist/gitlab_tools-0.3.1-py3-none-any.whl > dist/SHA256SUMS.txt
```

Windows PowerShell：

```powershell
$wheel = "dist\gitlab_tools-0.3.1-py3-none-any.whl"
...
```bash
git tag -a v0.3.1 -m "gitlab-tools v0.3.1"
